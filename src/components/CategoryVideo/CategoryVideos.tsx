...
type Props = State
const CategoryVideos = (props: Props) => {
  ...
  useEffect(() => {
    ...
    ActionCreators.ListCateNews(dispatch,"3");
                ...
                state.item ?
                  state.item.Results ?
                    state.item.Results.map((item: any, index: number) => {
                      return (
                        <>
                          <a style={{ backgroundColor: CategotyID==item.Id? '#FF5E14': '' }}  href={"../CategoryVideo/"+item.Title_Format_Url +'/'+item.Id }  className="btn">{item.Title}</a>
