...
type Props = State
const CategoryAudio = (props: Props) => {
  ...
  useEffect(() => {
    ...
    ActionCreators.ListCateNews(dispatch,"2");
  ...
  { console.log("statesisiisdsAudio", state) }
  return (
    <section>
                ...
                state.item ?
                  state.item.Results ?
                    state.item.Results.map((item: any, index: number) => {
                      return (
                        <>
                          <a style={{ backgroundColor: CategotyID==item.Id? '#FF5E14': '' }}  href={"../CategoryAudio/"+item.Title_Format_Url +'/'+item.Id }  className="btn">{item.Title}</a>
                        ...
                        <div id="divCategoryNews" className="col-sm-3 col-12">
                          <div className="card mb-4 shadow-sm">
                            ...
                            <div className="card-title">
                              <h4>
                                <a href={"../DetailVideo/"+item.Title_Format_Url +'/'+item.Id } className="card-text">{item.Title}</a>
                              </h4>
                            </div>
                            ...
                            <div className="DateNewsBottom d-flex justify-content-between align-items-center">
                              ...
                              <div className="btn-group">
                                <a href={"../DetailVideo/"+item.Title_Format_Url +'/'+item.Id } style={{ float: 'right' }}  >Chi tiết</a>
                              </div>
                            </div>
                          </div>
                        </div>
